...
void HTTPClient::clear()
{
    ...
    _payload.reset();
	//------------------------------
	detechResponse._Disposition = "";
	detechResponse._Type = "";
	detechResponse._Date= "";
	detechResponse._Location = "";
}
...
String HTTPClient::headers(String name)
{
	if (name.equals("Disposition")) return detechResponse._Disposition;
	if (name.equals("Type")) return detechResponse._Type;
	if (name.equals("Date")) return detechResponse._Date;
	if (name.equals("Location")) return detechResponse._Location;
...
{
	DEBUG_HTTPCLIENT("[HTTP-Client] bool HTTPClient::connect(void) run ... \n");
    ...
    _client->setTimeout(_tcpTimeout); DEBUG_HTTPCLIENT("[HTTP-Client] _client->setTimeout(_tcpTimeout); run .. \n");

	_client->connect(_host.c_str(), _port); DEBUG_HTTPCLIENT("[HTTP-Client] Flag 6 _client->connect(_host.c_str(), _port) ... \n");
...
#endif
	DEBUG_HTTPCLIENT("[HTTP-Client] Flag end ... \n");
    return connected();
}
        ...
        if(len > 0) {
            ...
            if(headerLine.startsWith("HTTP/1.")) {
				...
				if (headerName.equalsIgnoreCase("content-type")) {
					detechResponse._Type = headerValue;
				}

				if (headerName.equalsIgnoreCase("content-disposition")) {
					detechResponse._Disposition = headerValue;
				}

				if (headerName.equalsIgnoreCase("location")) {
					detechResponse._Location = headerValue;
				}

				if (headerName.equalsIgnoreCase("date")) {
					detechResponse._Date = headerValue;
				}
